...
(function () {
  ...
  /* ---------- 全局加载遮罩：长时间等待可撤销（加载经文/抄本、生成 PDF） ---------- */
  var _loadingEl = null, _loadingCancel = null;
  function hideLoading() {
    if (_loadingEl) _loadingEl.classList.add('hidden');
    _loadingCancel = null;
  }
  // text：提示文案；onCancel：取消回调（不传则不显示取消按钮）。返回 {update, hide}
  function showLoading(text, onCancel) {
    if (!_loadingEl) {
  ...
  function createDualView(mount, ctx, withToggle) {
    ...
    var api = {
      ...
      render: function (cells, total, title, onDone) {
    ...
    function renderActive(onDone) {
      // 分块渲染：大作品逐字/整纸生成耗时，用加载进度条反馈；快速切换时旧任务自动作废
      api._renderSeq = (api._renderSeq || 0) + 1;
      var seq = api._renderSeq;
      var n = (api.cells || []).length;
      var ui = null, cancelled = false;
      if (n > 40) {
        ui = showLoading(api.view === 'sheet' ? '正在生成整纸…' : '正在生成逐字…', function () {
          cancelled = true; api._renderSeq++;
        });
        ui.progress(0, 100);
      }
      var hooks = {
        progress: function (f) { if (ui && !cancelled) ui.progress(Math.round(f * 100), 100); },
        cancelled: function () { return cancelled || seq !== api._renderSeq; },
        done: function () {
          if (ui) ui.hide();
          if (onDone) { try { onDone(!cancelled && seq === api._renderSeq); } catch (e) {} }
        }
      };
      if (api.view === 'char') renderPager(charBox, api, ctx, hooks);
      else renderSheet(sheetBox, api, ctx, hooks);
    }
  ...
  // hooks: {progress(frac), done(), cancelled()} —— 分块生成，大作品不卡界面
  function renderPager(box, api, ctx, hooks) {
    hooks = hooks || {};
    ...
    var i = 0, CHUNK = 12;
    (function step() {
      if (hooks.cancelled && hooks.cancelled()) return;
      var end = Math.min(cells.length, i + CHUNK);
      for (; i < end; i++) {
        (function (c, idx) {
          ...
          if (ctx.onTap) {
            ...
            card.addEventListener('click', function () { ctx.onTap(c.pos); });
          }
          var cap = mkEl('div', 'list-cap');
          cap.textContent = '第 ' + (idx + 1) + ' / ' + cells.length + ' 字 · ' + c.ch;
          row.appendChild(card); row.appendChild(cap);
          list.appendChild(row);
          api.cardByPos[c.pos] = card;
        })(cells[i], i);
      }
      if (hooks.progress) { try { hooks.progress(i / cells.length); } catch (e) {} }
      if (i < cells.length) {
        setTimeout(step, 0);
      } else {
        // 仪式燃烧跟随：把正在烧的字滚进视野
        api._go = function (n) {
          var row = list.children[Math.max(0, Math.min(cells.length - 1, n))];
  ...
  // 字图与 PDF 同取景（renderCropped 紧裁剪，大字满格）；分块生成，hooks 同 renderPager
  function renderSheet(box, api, ctx, hooks) {
    hooks = hooks || {};
    box.innerHTML = ''; api.cardByPos = {};
    var cells = api.cells || [];
    if (!cells.length) {
    ...
    (function step() {
        ...
        (function (it) {
          ...
          if (it.ash) {
            ...
            if (ctx.onTap) {
              ...
              d.addEventListener('click', function () { ctx.onTap(it.pos); });
            }
          }
      ...
      if (hooks.progress) { try { hooks.progress(i / items.length); } catch (e) {} }
      if (i < items.length) {
        setTimeout(step, 0);
      } else {
  ...
  function refreshWorkCells() {
    ...
    dual.render(cells, dual.total, dual.title, function (ok) {
      if (ok && idx >= 0 && dual.view === 'char') dual.goTo(idx); // 逐字：滚回刚改的字
    });
  }
